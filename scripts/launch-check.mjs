...
console.log("\nindexable only on the real domain");
{
  ...
  const www = await headWithHost("/venues", "www.domisignature.com");
  check("www redirects to the apex",
    String(www.status) === "308" && (www.headers["location"] ?? "").startsWith("https://domisignature.com/venues"),
    `${www.status} ${www.headers["location"] ?? "(no location)"}`);
