...
/**
 * Editorial venue card. Deliberately not a boxed "card" — no shadow at rest, no
 * rounded corners, and the only frame is the plate's. Plate, name, one line,
 * capacity.
 *
 ...
 *
 * Stage 6: the plate lifts. On a fine pointer's hover of the card, or while its
 * link holds keyboard focus, the plate rises and a soft shadow appears beneath
 * it (`<Plate lift>`). The photograph's hover zoom is gone: a plate rising
 * while its picture grows inside it is two gestures for one. The card is a div
 * around the link, not the link itself — the lift follows a `.group` that
 * `:has(:focus-visible)`, and :has() looks only at descendants, so the group
 * must hold the focused link rather than be it.
 */
export function VenueCard({
  ...
  return (
    <div className={cn("group", className)}>
      {/* h-full: the link still fills the card's grid cell, so its hit area
          and its focus ring are the ones it had as the grid item itself. */}
