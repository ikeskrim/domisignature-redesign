      ...
      <div className="relative mx-auto w-full max-w-[104rem] px-gutter">
        <Reveal>
          <div className="flex items-center gap-6">
            <span className="eyebrow">Crete, Greece</span>
        ...
        <TextReveal
          ...
          className="mt-14 font-display text-[clamp(2.5rem,7.2vw,7.5rem)] font-light leading-[0.95] tracking-[-0.028em] text-[var(--text-primary)]"
          delay={0.05}
        />
            ...
            <ul className="mt-8 space-y-4">
              {disciplines.map((item, i) => (
                <Reveal as="li" key={item} delay={0.04 + i * 0.05} y={14}>
                  <span className="text-[0.98rem] leading-relaxed text-[var(--text-primary)]">{item}</span>
                </Reveal>
              ))}
            </ul>
          </div>
