            ...
            {team.map((member, i) => (
              <li key={member.name} className={i === 1 ? "sm:mt-14" : undefined}>
                ...
                <Reveal delay={0.1 + i * 0.09}>
                  <h3 className="mt-6 font-display text-[1.6rem] font-light leading-tight text-[var(--text-primary)]">
                    {member.name}
                  </h3>
                  <p className="mt-2 text-[0.6875rem] uppercase tracking-[0.2em] text-[var(--text-tertiary)]">
                    {member.role}
                  </p>
                </Reveal>
              </li>
